...
Then we will summarize the data and try to predict the number of shares is two via linear regression models, a random forest model, and a boosting model. Lastly we will compare the four models and declare a winner (the model with the lowest root mean squared error).  
...
The data contains 61 variables (58 predictive attributes, 2 non-predictive, 1 goal field) and we are interested in the number of shares (goal) of the online news. 
...
* `corrplot` : corrplot provides a visual exploratory tool on correlation matrix
...
Firstly we set up a relative path, then We read in the OnlineNewsPopularity.csv data file found [here](https://archive.ics.uci.edu/ml/datasets/Online+News+Popularity) using `read_csv()`.
...
We now subset the data for the data channel of interest: `r params$channel`: Is data channel '`r str_to_title(params$label)`'?
...
Now we split the data set we’ve created into a 70% training set called "ChannelTrain" and a 30% testing set called "ChannelTest". We will be using the `caret` package with p = 0.7 to achieve the 70% training and 30% testing split.
...
We begin by doing some exploratory data analysis(EDA). It should be noted that for the following EDA and summaries we use our training data.


lets begin by Plotting the correlation between a few notable numeric variables.
...
From the above plot we note that none of our selected variables appear to share any meaningful correlation with the number of shares. However number of words in the content and number of images appear to have a moderate/strong positive correlation.

Next, lets create a few scatter plots to get a better visual of this correlation.

We start by examining Trend of Number of words in the content vs Number of shares, Trend of Number of images vs Number of shares, Trend of Number of videos vs Number of shares and Trend of Number of links vs Number of shares.
...
From the four plots above we can see that the number of words in the content, number or images, number of videos and the number of links in the online news all share a weak or negligible positive correlation to the number of shares as they all show an almost horizontal tend line.
...
The above plot shows that there is a moderate/strong positive correlation between the number of words in the content and number of images.
...
The above table shows that for our training data `r weekend[2]` online news articles were shared during the weekend and `r weekend[1]` were shared during the week.
...
From the above table we see that for our training data `r mon_tue[2, 1]` online news articles were shared on a Monday, `r mon_tue[1, 2]` were shared on a Tuesday and `r mon_tue[1, 1]` were shared the rest of the week. 

Since Monday is considered the beginning of the work week, it is interesting to see how monday "stacks up with the other days. For the contingency tables below,  we can use a similar interpretation as we did for the previous table.
...
```
Next let us examime the number of shares for the different days of the week using a contingency table and a bar plot
...
Both the contingency tables and bar plot above concur that most of the articles were shared on a Wednesday and the least number of articles were shared on a Saturday.
...
The case of one predictor variable is called simple linear regression and the case where there are more than one predictor variables is called multiple linear regression.
...
We model the number of shares using multiple linear regression, summarize and predict on the testing data. and use the `postResample()` function to get useful metrics. The selected independent variables for this model are;
...
The idea behind the boosting tree model is to train our tree slowly in a sequential manner so each tree that is created will be based on the previous one with predictions updated. More formally, boosting (for regression trees) involves
